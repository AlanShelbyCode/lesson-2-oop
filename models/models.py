...
class Car(Model):
    ...
    file = 'car.json'
    def __init__(self,car_user_id,car_model,car_year,car_color):
        users = User.get_file_data(User.file)
        count = 0
        for user in users:
            if str(user['id']) == str(car_user_id):
                count += 1
        print(count)
        if count != 0:
            self.car_user_id = car_user_id
        else:
            self.car_user_id = None
            print('We dont find a user with this id')
